...
namespace UnitTestProject
    {
    public class PerformanceTestsForRecommendations
        {

        private readonly ITestOutputHelper _output;
        private const string BaseUrl = "http://192.168.1.237:5000"; 
        private const string Endpoint = "api/Recommendations/byAppoint/{4afd274b-9d5b-48f7-b2fa-10c590e66f8d}";

        public PerformanceTestsForRecommendations(ITestOutputHelper output)
            {
            _output = output;
            }
        ...
        [Fact]
        public async Task ApiPerformance_ShouldMeet90PercentUnder1_5SecondsForRecommendations()
            {
            var client = new RestClient(BaseUrl);
            var request = new RestRequest(Endpoint, Method.Get);
